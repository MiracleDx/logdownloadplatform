...
import com.log.download.platform.service.CallLanJingInterfaceService;
...
public class LogController {
	...
	@Resource
	private CallLanJingInterfaceService callLanJingInterfaceService;
	...
	public ServerResponse<List<LogDetailVO>> queryLogDetails(@RequestBody QueryLogDetailDTO queryLogDetailDTO) {
		...
		//拼接快速执行脚本的参数
		String params = callLanJingInterfaceService.getFastExecuteScriptParams(queryLogDetailDTO.getLabel(), queryLogDetailDTO.getIps(), script_id, paramArr);
		//执行脚本，并获取结果
		JSONObject result = callLanJingInterfaceService.callLanJingInterface("",params);
		//如果执行成功，查询执行日志
		if(Boolean.valueOf(result.getJSONObject("result").toString())){
			...
			//查询日志
			String params_log = callLanJingInterfaceService.getJobInstanceLogParams(queryLogDetailDTO.getLabel(), job_instance_id);
			JSONObject result_log = callLanJingInterfaceService.callLanJingInterface("", params_log);
			//获取log日志
